...
class _EditTodoState extends State<EditTodo> {
  ...
  @override
  void initState() {
    fetchTodoDetails(); //set the initial value of text field
    super.initState();
  }
  ...
  Future<void> fetchTodoDetails() async {
    ...
    final todo = await database!
        .query('todos', where: 'id = ?', whereArgs: [widget.todoId]);

    if (todo.isNotEmpty) {
      setState(() {
        titleInput.text = todo[0]['title'].toString();
        descInput.text = todo[0]['description'].toString();
        dateInput.text = todo[0]['date'].toString();
        timeInput.text = todo[0]['time'].toString();
      });
    }
  }

  Future<void> updateTodoDetails(
      int id, String title, String desc, String date, String time) async {
    final database = await DatabaseHelper().database;
    await database!
        .update(
          'todos',
          ...
          children: [
            ...
            Container(
              ...
              child: ElevatedButton(
                onPressed: () {
                  updateTodoDetails(widget.todoId, titleInput.text,
                      descInput.text, dateInput.text, timeInput.text);
                },
                child: Padding(
